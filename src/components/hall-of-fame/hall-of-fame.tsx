  ...
  return (
    <div className="space-y-7">
      ...
      <section ref={cardRef} className="zellige-card rounded-[32px] bg-morocco px-5 pb-6 pt-7 text-ivory shadow-card">
        ...
        <div className="mt-8 grid grid-cols-3 items-end gap-2">{[podium[1], podium[0], podium[2]].map((person, index) => person ? <div key={person.id} className={`relative rounded-t-2xl bg-ivory/10 p-3 text-center ${index === 1 ? "min-h-44 pt-5" : "min-h-36 pt-4"}`}><PodiumIcon rank={index === 1 ? 1 : index === 0 ? 2 : 3} /><strong className="mt-2 block truncate text-sm">{person.name}</strong><span className="mt-1 block font-display text-2xl font-bold text-sand">{person.total}</span></div> : <div key={index} />)}</div>
      </section>
